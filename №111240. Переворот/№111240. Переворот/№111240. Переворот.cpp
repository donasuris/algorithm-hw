...
struct Node {
	...
	int Min(Node* a, Node* b) {
		if (a == NULL && b == NULL) return 1e9 + 1;
		else if (a == NULL) return b->min_val;
		else if (b == NULL) return a->min_val;
		else return min(a->min_val, b->min_val);
	}
...
public:
	...
	void Split(int idx, Node*& L, Node*& R, Node* root) {
		...
		if (sL <= idx) {
			L = root;
			if (root->right != NULL) Split(idx - sL, root->right, R, root->right);
			else R = NULL;
			root->Recalc();
			...
			R = root;
			if (root->left != NULL) Split(idx, L, root->left, root->left);
			else L = NULL;
			root->Recalc();
		}
	}
	...
	void PushBack(int data) {
		int priority = rand();
		if (root == NULL) root = new Node(data, priority);
	...
	int& operator[] (int idx) {
		...
		if (curr == NULL) {
			...
			else throw out_of_range("no item with that index");
		}
	}
};
...
int main()
{
	...
	while (m-->0) {
		cin >> cmd >> a >> b;
		if (cmd == 1) tree.Reverce(a, b);
		else if (cmd == 2) cout << tree.MinOf(a, b) << endl;
	}
